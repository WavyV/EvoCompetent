...
public class player51 implements ContestSubmission
{
  ...
  private int evaluations_limit_;
    ...
    individuals = rank_population(individuals);
      ...
      double [] parent2 = new double[D+1];
      double [][] children = new double[N][D+1];
        ...
        do {
          ...
          if(dummy_array[random_index] == 0){
            ...
            picked = true;
				...
				// If we have picked 4 parents, then go do roulette wheel, else pick another parent
        if(parent_number == 1){
          pparent1 = random_index;
          parent_number++;
        } 
          else if(parent == 2){
          pparent2 = random_index;
          parent_number++;
        }
          else if(parent == 3){
          pparent3 = random_index;
          parent_number++;
        }
          else {
          pparent4 = random_index;
          parent_number = 1;

          // Carry out roulette
          // Add up the index of each potential parent so get the sum of fitness (as individuals index correspondes to their ranking/fitness) 
          //[is better to do this wuth acutal fitness but don't know how to access it]
          int sum_of_fitness = pparent1 + pparent2 +pparent3 + pparent4;
          // Get proportion each potential parent has of the roulette by dividing fitness by sum of fitness, with addition of previous potential parents proportion (pp4 should equal sum_of fitness)
          int pp1 = pparent1/sum_of_fitness;
          int pp2 = pp1 + pparent2/sum_of_fitness;
          int pp3 = pp2 + pparent3/sum_of_fitness;
          int pp4 = pp3 + pparent4/sum_of_fitness;
          
          //run roulette twice on same parents so we get both parents from the same sample (could be that the both parents are the same)
          for(int j=1; j<3; j++){
          int random_selection = rnd_.nextInt(sum_of_fitness);
          if(j ==1){

          if(random_selection < pp1){
            parent1 = individuals[pp1];
          }
            else if(random_selection < pp2){
              parent1 = individuals[pp2];
            }
            else if(random_selection < pp3){
              parent1 = individuals[pp3];
            }
            else if(random_selection < pp4){
              parent1 = individuals[pp4];
            }
          }
          else{
